...
{
	return mSource < other.mSource && mSessionId < other.mSessionId;
}
...
void Receiver::setAddedFn( TypeFn<TuioType> callback )
{
	...
	if( found != mHandlers.end() ) {
		...
		mReceiver->setListener( address, std::bind( &detail::TypeHandlerBase::handleMessage, created, std::placeholders::_1 ) );
	}
}
...
void Receiver::setupWindowReceiver( ci::app::WindowRef window )
{
	...
	mReceiver->setListener( address, std::bind( &detail::TypeHandlerBase::handleMessage, handler, std::placeholders::_1 ) );
...
{
	if( std::is_same<T, Cursor2d>::value )
		return "/tuio/2Dcur";
	else if( std::is_same<T, Cursor25d>::value )
		return "/tuio/25Dcur";
	else if( std::is_same<T, Cursor3d>::value )
		return "/tuio/3Dcur";
	else if( std::is_same<T, Object2d>::value )
		return "/tuio/2Dobj";
	else if( std::is_same<T, Object25d>::value )
		return "/tuio/25Dobj";
	else if( std::is_same<T, Object3d>::value )
		return "/tuio/3Dobj";
	else if( std::is_same<T, Blob2d>::value )
		return "/tuio/2Dblb";
	else if( std::is_same<T, Blob25d>::value )
		return "/tuio/25Dblb";
	else if( std::is_same<T, Blob3d>::value )
		return "/tuio/3Dblb";
	else
...
void TypeHandler<TuioType>::handleMessage( const osc::Message &message )
{
	...
	if( messageType == "source" ) {
		...
		auto it = find_if( currentBegin, currentEnd, [sessionId]( const TuioType &type ) {
		...
		if( !this->mAdded.empty() ) {
			if( mAddCallback ) {
				for( auto &added : this->mAdded ) {
					auto found = find_if( currentBegin, currentEnd, [added]( const TuioType &type ) {
		...
		if( !this->mUpdated.empty() ) {
			if( mUpdateCallback ) {
				for( auto &updated : this->mUpdated ) {
					auto found = find_if( currentBegin, currentEnd, [updated]( const TuioType &type ) {
...
void WindowCursorHandler::handleFseq( int32_t frame )
{
	...
	if( frame == -1 || delta_frame > 0 ) {
		...
		if( !this->mAdded.empty() ) {
			for( auto &added : this->mAdded ) {
				auto found = find_if( currentBegin, currentEnd, [added]( const Cursor2d &type ) {
					return added == type.getSessionId();
				} );
				if( found != currentEnd )
		...
		if( !this->mUpdated.empty() ) {
			for( auto &updated : this->mUpdated ) {
				auto found = find_if( currentBegin, currentEnd, [updated]( const Cursor2d &type ) {
					return updated == type.getSessionId();
				} );
				if( found != currentEnd )
