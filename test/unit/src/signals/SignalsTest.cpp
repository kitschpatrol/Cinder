...
TEST_CASE( "signals/Signals" )
{
	SECTION( "Basic signal tests" )
	{
		...
		bool success;
		success = conn1.disconnect();
		REQUIRE( success == true );
		REQUIRE( sig1.getNumSlots() == 2 );
		success = conn1.disconnect();
		REQUIRE( success == false );
		REQUIRE( sig1.getNumSlots() == 2 );
		success = conn2.disconnect();
		REQUIRE( success == true );
		REQUIRE( sig1.getNumSlots() == 1 );
		success = conn3.disconnect();
		REQUIRE( success == true );
		REQUIRE( sig1.getNumSlots() == 0 );
		success = conn3.disconnect();
		REQUIRE( success == false );
		REQUIRE( sig1.getNumSlots() == 0 );
		success = conn2.disconnect();
