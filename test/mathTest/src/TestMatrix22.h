...
template <typename T>
void TestMatrix22( std::ostream &os )
{
	...
	// Matrix22( T d0, T d1, T d2, T d3 );
	{
		...
		T    data[4] = { 1, 2, 3, 4 };
		MatT c0( 1, 2, 3, 4 );
		result = ( ( data[0] == c0.m[0] ) && ( data[1] == c0.m[1] ) && ( data[2] == c0.m[2] ) && ( data[3] == c0.m[3] ) );
		os << ( result ? "passed" : "FAILED" ) << " : "
		   << "Matrix22( T d0, T d1, T d2, T d3 );"
		   << "\n";
	}
	...
	// template< typename FromT > Matrix22( const Matrix22<FromT>& src );
	{
		...
		result = ( ( dmat0 == Matrix22<double>( fmat0 ) ) && ( fmat0 == Matrix22<float>( dmat0 ) ) );
		os << ( result ? "passed" : "FAILED" ) << " : "
		   << "template< typename FromT > Matrix22( const Matrix22<FromT>& src );"
		   << "\n";
	}
	...
	// operator T*() { return (T*)m; }
	{
		...
		MatT c0( 1, 2, 3, 4 );
		T *  data = (T *)c0;
		result = ( ( c0.m[0] == data[0] ) && ( c0.m[1] == data[1] ) && ( c0.m[2] == data[2] ) && ( c0.m[3] == data[3] ) );
		os << ( result ? "passed" : "FAILED" ) << " : "
		   << "operator T*() { return (T*)m; }"
		   << "\n";
	}
	...
	// operator const T*() const { return (const T*)m; }
	{
		...
		result = ( ( c0.m[0] == const_data[0] ) && ( c0.m[1] == const_data[1] ) && ( c0.m[2] == const_data[2] ) && ( c0.m[3] == const_data[3] ) );
		os << ( result ? "passed" : "FAILED" ) << " : "
		   << "operator const T*() const { return (const T*)m; }"
		   << "\n";
	}
	...
	// bool equalCompare( const Matrix22<T>& rhs, T epsilon ) const;
	{
		...
		result = ( false == ( m0.equalCompare( c0, epsilon ) ) && true == ( m1.equalCompare( c0, epsilon ) ) );
		os << ( result ? "passed" : "FAILED" ) << " : "
		   << "bool equalCompare( const Matrix22<T>& rhs, T epsilon ) const;"
		   << "\n";
	}
	...
	// const T& at( int row, int col ) const;
	{
		bool result = false;
		MatT c0( 1, 2, 3, 4 );
		result = ( ( fabs( 1 - c0.at( 0, 0 ) ) < EPSILON ) && ( fabs( 2 - c0.at( 1, 0 ) ) < EPSILON ) && ( fabs( 3 - c0.at( 0, 1 ) ) < EPSILON ) && ( fabs( 4 - c0.at( 1, 1 ) ) < EPSILON ) );
		os << ( result ? "passed" : "FAILED" ) << " : "
		   << "const T& at( int row, int col ) const;"
		   << "\n";
	}
	...
	// Vec2<T> getColumn( int col ) const;
	{
		bool result = false;
		MatT m0( 1, 2, 3, 4 );
		result = ( ( Vec2<T>( (T)1, (T)2 ) == m0.getColumn( 0 ) ) && ( Vec2<T>( (T)3, (T)4 ) == m0.getColumn( 1 ) ) );
		os << ( result ? "passed" : "FAILED" ) << " : "
		   << "Vec2<T> getColumn( int col ) const;"
		   << "\n";
	}
	...
	// Vec2<T> getRow( int row ) const;
	{
		bool result = false;
		MatT m0( 1, 2, 3, 4 );
		result = ( ( Vec2<T>( (T)1, (T)3 ) == m0.getRow( 0 ) ) && ( Vec2<T>( (T)2, (T)4 ) == m0.getRow( 1 ) ) );
		os << ( result ? "passed" : "FAILED" ) << " : "
		   << "Vec2<T> getRow( int row ) const;"
		   << "\n";
	}
	...
	// void invert (T epsilon = FLT_MIN );
	{
		bool result = false;
		MatT c0(
		    (T)-67114.093959732, (T)0.3355704698, (T)0.5033557047, (T)-1.68e-8 );
		MatT m0(
		    (T)0.0000001, (T)2, (T)3, (T)400000 );
		m0.invert();
		result = ( c0 == m0 );
		os << ( result ? "passed" : "FAILED" ) << " : "
		   << "void invert (T epsilon = FLT_MIN );"
	...
	{
		bool result = false;
		MatT c0( (T)2.182514, (T)-0.486452, (T)4.606950, (T)-1.943200 );
		MatT m0( 1, 2, 3, 4 );
	...
	{
		bool result = false;
		MatT c0( (T)0.241922, (T)-0.970296, (T)0.970296, (T)0.241922 );
