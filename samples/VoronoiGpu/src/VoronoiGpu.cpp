...
ci::Surface32f calcDiscreteVoronoiGpu( const std::vector<ci::ivec2> &points, int width, int height )
{
	...
	format.setColorTextureFormat( gl::Texture::Format().target( GL_TEXTURE_RECTANGLE_ARB ).minFilter( GL_NEAREST ).magFilter( GL_NEAREST ).internalFormat( GL_RGB32F_ARB ).wrap( GL_CLAMP_TO_EDGE ) );
	gl::FboRef fbo[2];
