...
DeferredShadingAdvancedApp::DeferredShadingAdvancedApp()
{
	...
	for( size_t i = 0; i < mNumBouncingLights; ++i ) {
		...
		mLights.push_back( Light().colorDiffuse( c ).intensity( 0.5f ).radius( 0.1f ).volume( 5.0f ) );
	}
	...
	for( size_t i = 0; i < 5; ++i, t += d ) {
		...
		mLights.push_back( Light().colorDiffuse( ColorAf( 0.85f, 0.7f, 1.0f, 1.0f ) ).intensity( 1.0f ).position( p ).radius( 0.1f ).volume( 3.0f ) );
	}

	// Primary light
	mLights.push_back( Light().colorDiffuse( ColorAf( 1.0f, 0.95f, 0.9f, 1.0f ) ).intensity( 1.2f ).radius( 0.3f ).volume( 15.0f ) );

	// Set up materials
	mMaterials.push_back( Material().colorDiffuse( Colorf::white() ).colorSpecular( Colorf::white() ).shininess( 300.0f ) ); // Sphere
	mMaterials.push_back( Material().colorAmbient( ColorAf::gray( 0.18f ) ).colorDiffuse( Colorf::gray( 0.6f ) ).colorSpecular( Colorf::gray( 0.22f ) ).shininess( 20.0f ) ); // Room
	mMaterials.push_back( Material().colorAmbient( ColorAf::black() ).colorDiffuse( Colorf::black() ).colorEmission( Colorf::white() ).shininess( 100.0f ) ); // Light
...
void DeferredShadingAdvancedApp::createBatches()
{
	...
	int32_t         version = 330;
	gl::GlslProgRef aoComposite = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragAoComposite ).define( "TEX_COORD" ) );
	gl::GlslProgRef aoHbao = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragAoHbaoAo ).define( "TEX_COORD" ) );
	gl::GlslProgRef aoHbaoBlur = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragAoHbaoBlur ).define( "TEX_COORD" ) );
	gl::GlslProgRef aoSaoAo = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragAoSaoAo ) );
	gl::GlslProgRef aoSaoBlur = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragAoSaoBlur ).define( "TEX_COORD" ) );
	gl::GlslProgRef aoSaoCsz = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragAoSaoCsz ).define( "TEX_COORD" ) );
	gl::GlslProgRef bloomBlur = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragBloomBlur ).define( "TEX_COORD" ) );
	gl::GlslProgRef bloomComposite = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragBloomComposite ).define( "TEX_COORD" ) );
	gl::GlslProgRef bloomHighpass = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragBloomHighpass ).define( "TEX_COORD" ) );
	gl::GlslProgRef debug = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragDeferredDebug ).define( "TEX_COORD" ).define( "NUM_MATERIALS", numMaterials ) );
	gl::GlslProgRef emissive = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragDeferredEmissive ).define( "TEX_COORD" ).define( "NUM_MATERIALS", numMaterials ) );
	gl::GlslProgRef gBuffer = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertDeferredGBuffer ).fragment( fragDeferredGBuffer ) );
	gl::GlslProgRef gBufferInvNorm = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertDeferredGBuffer ).fragment( fragDeferredGBuffer ).define( "INVERT_NORMAL" ) );
	gl::GlslProgRef gBufferInst = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertDeferredGBuffer ).fragment( fragDeferredGBuffer ).define( "INSTANCED_MODEL" ) );
	gl::GlslProgRef gBufferInstLS = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertDeferredGBuffer ).fragment( fragDeferredGBuffer ).define( "INSTANCED_LIGHT_SOURCE" ).define( "NUM_LIGHTS", numLights ) );
	gl::GlslProgRef lBufferLight = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertDeferredLBufferLight ).fragment( fragDeferredLBufferLight ).define( "NUM_MATERIALS", numMaterials ).define( "NUM_LIGHTS", numLights ) );
	gl::GlslProgRef lBufferShadow = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragDeferredLBufferShadow ) );
	gl::GlslProgRef shadowMapInst = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragDeferredShadowMap ).define( "INSTANCED_MODEL" ) );
	gl::GlslProgRef postColor = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragPostColor ).define( "TEX_COORD" ) );
	gl::GlslProgRef postDof = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragPostDof ).define( "TEX_COORD" ) );
	gl::GlslProgRef postFog = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragPostFog ).define( "TEX_COORD" ) );
	gl::GlslProgRef postFxaa = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragPostFxaa ).define( "TEX_COORD" ) );
	gl::GlslProgRef rayComposite = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragRayComposite ).define( "TEX_COORD" ) );
	gl::GlslProgRef rayOcclude = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragRayOcclude ).define( "TEX_COORD" ) );
	gl::GlslProgRef rayScatter = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragRayScatter ).define( "TEX_COORD" ) );
	...
	gl::GlslProgRef shadowMap = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragDeferredShadowMap ) );
	...
	mBatchGBufferIcosahedron = gl::Batch::create( icosahedron, gBufferInst, { { geom::Attrib::CUSTOM_0, "vInstanceModelMatrix" }, { geom::Attrib::CUSTOM_1, "vInstanceNormalMatrix" } } );
	mBatchGBufferSphere = gl::Batch::create( sphereInst, gBufferInst, { { geom::Attrib::CUSTOM_0, "vInstanceModelMatrix" }, { geom::Attrib::CUSTOM_1, "vInstanceNormalMatrix" } } );
	mBatchShadowMapIcosahedron = gl::Batch::create( icosahedron, shadowMapInst, { { geom::Attrib::CUSTOM_0, "vInstanceModelMatrix" }, { geom::Attrib::CUSTOM_1, "vInstanceNormalMatrix" } } );
	mBatchShadowMapSphere = gl::Batch::create( sphereInst, shadowMapInst, { { geom::Attrib::CUSTOM_0, "vInstanceModelMatrix" }, { geom::Attrib::CUSTOM_1, "vInstanceNormalMatrix" } } );
...
void DeferredShadingAdvancedApp::resize()
{
	...
	// 2 GL_COLOR_ATTACHMENT2	Encoded normals
	{
		...
		mTextureFboGBuffer[1] = gl::Texture2d::create( sz.x, sz.y, gl::Texture2d::Format().internalFormat( GL_R8I ).magFilter( GL_NEAREST ).minFilter( GL_NEAREST ).wrap( GL_CLAMP_TO_EDGE ).dataType( GL_BYTE ) );
		mTextureFboGBuffer[2] = gl::Texture2d::create( sz.x, sz.y, gl::Texture2d::Format().internalFormat( GL_RG16F ).magFilter( GL_NEAREST ).minFilter( GL_NEAREST ).wrap( GL_CLAMP_TO_EDGE ).dataType( GL_BYTE ) );
		gl::Fbo::Format fboFormat;
	...
	if( mAo != Ao_None ) {
		...
		for( size_t i = 0; i < 2; ++i ) {
			mTextureFboAo[i] = gl::Texture2d::create( sz.x, sz.y, gl::Texture2d::Format().internalFormat( GL_RG32F ).magFilter( GL_LINEAR ).minFilter( GL_LINEAR ).wrap( GL_CLAMP_TO_EDGE ).dataType( GL_FLOAT ) );
		...
		if( mAo == Ao_Sao ) {
			...
			mFboCsz = gl::Fbo::create( mFboGBuffer->getWidth(), mFboGBuffer->getHeight(), gl::Fbo::Format().disableDepth().colorTexture( cszTextureFormat ) );
			const gl::ScopedFramebuffer scopedFramebuffer( mFboCsz );
			const gl::ScopedViewport    scopedViewport( ivec2( 0 ), mFboCsz->getSize() );
			gl::clear();
		}
	...
	// Create shadow map buffer
	{
		...
		mFboShadowMap = gl::Fbo::create( sz, sz, gl::Fbo::Format().depthTexture( depthTextureFormat ) );
	...
	mShadowCamera.setPerspective( 120.0f, mFboShadowMap->getAspectRatio(), mCamera.getNearClip(), mCamera.getFarClip() );
...
void DeferredShadingAdvancedApp::update()
{
	...
	if( mAoPrev != mAo || mEnabledRayPrev != mEnabledRay || mHighQualityPrev != mHighQuality ) {
		resize();
		mAoPrev = mAo;
		mEnabledRayPrev = mEnabledRay;
		mHighQualityPrev = mHighQuality;
...
CINDER_APP( DeferredShadingAdvancedApp, RendererGl( RendererGl::Options().version( 3, 3 ) ), []( App::Settings *settings ) {
