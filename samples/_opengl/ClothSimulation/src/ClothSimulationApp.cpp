      ...
      mCam( getWindowWidth(), getWindowHeight(), 20.0f, 0.01f, 1000.0f )
{
	...
	vec3 eye = vec3( sin( mCurrentCamRotation ) * 140.0f, 0, cos( mCurrentCamRotation ) * 140.0f );
...
void ClothSimulationApp::setupGlsl()
{
	...
	std::vector<std::string> feedbackVaryings( { "tf_position_mass",
	    "tf_velocity" } );
...
CINDER_APP( ClothSimulationApp, RendererGl(), [&]( App::Settings *settings ) {
