...
void ParticleSphereCSApp::setup()
{
	...
	for( unsigned int i = 0; i < particles.size(); ++i ) { // assign starting values to particles.
	...
	try {
		mRenderProg = gl::GlslProg::create( gl::GlslProg::Format().vertex( loadAsset( "particleRender.vert" ) ).fragment( loadAsset( "particleRender.frag" ) ).attribLocation( "particleId", 0 ) );
	}
