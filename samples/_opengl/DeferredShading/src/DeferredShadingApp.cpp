...
DeferredShadingApp::DeferredShadingApp()
{
	...
	// Set up lights
	mLights.push_back( Light().colorDiffuse( ColorAf( 0.95f, 1.0f, 0.92f, 1.0f ) ).intensity( 0.8f ).position( vec3( 0.0f, 0.0f, 0.0f ) ).radius( 0.1f ).volume( 15.0f ) );
	for( size_t i = 0; i < 8; ++i ) {
		const float t = ( (float)i / 8.0f ) * 0.2f;
		mLights.push_back( Light().colorDiffuse( ColorAf( 0.75f + t * 0.5f, 0.92f - t, 0.7f + t * t, 1.0f ) ).intensity( 0.6f ).radius( 0.05f ).volume( 5.0f ) );
	}
...
// Create GLSL programs
	...
	int32_t version = 330;
#endif
	const gl::GlslProgRef debug = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragDebug ) );
	const gl::GlslProgRef fxaa = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragFxaa ) );
	const gl::GlslProgRef gBuffer = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertGBuffer ).fragment( fragGBuffer ) );
	const gl::GlslProgRef lBuffer = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertPassThrough ).fragment( fragLBuffer ) );
	const gl::GlslProgRef shadowMap = loadGlslProg( gl::GlslProg::Format().version( version ).vertex( vertShadowMap ).fragment( fragShadowMap ) );
