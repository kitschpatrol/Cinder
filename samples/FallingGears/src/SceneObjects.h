...
struct SceneObject {
	SceneObject( AudioController *audio )
	    : mAudio( audio ) {}
	virtual ~SceneObject() {}
...
struct Island : public SceneObject {
	Island( AudioController *audio )
	    : SceneObject( audio ), mVibrationLevel( 0 ) {}
	void setupGeometry();
...
struct Wall : public SceneObject {
	Wall( AudioController *audio )
	    : SceneObject( audio ), mVibrationLevel( 0 ), mFundamentalMidi( 0 ), mWidth( 0 ) {}
	void handleCollision( const Gear *gear, const ci::vec2 &contactPoint ) override;
	void draw() override;
